...
<head>
  ...
  <meta name="description" content="Joystick with VvvebJs">
  <meta name="author" content="VvvebJs">
  <link rel="icon" href="favicon.ico">
  <base href="">
  <title>Joystick with VvvebJs</title>
...
<div id="top-panel">
  ...
  <div class="btn-group mr-3" role="group">
    <button class="btn btn-light" title="Export (Ctrl + E)" id="save-btn" data-vvveb-action="saveAjax" data-vvveb-shortcut="ctrl+e" data-id="{{ $page->id }}" data-routeurl="{{ $route_url }}"> <i class="la la-save"></i> </button>
...
<form method="POST" action="/save-html-{{ $route_url }}">
  @csrf
</form>
...
<script>
  $(document).ready(function() 
  {
    ...
    [
      ...
      <?php
          foreach (glob("vvvebJs/templates/*.html") as $file) :
              if (in_array($file, array('new-page-blank-template.html', 'editor.html')))
                  continue; //skip template files

              $pathInfo = pathinfo($file);
              // echo '{name:"' . ucfirst($pathInfo['filename']) . '",' .
                    // 'title:"' . ucfirst($pathInfo['filename']) . '",' .
                    // 'url: "' . '/vvvebJs/templates/'.$pathInfo['basename'] . '"},';
      ?>
      {name:"<?= ucfirst($pathInfo['filename']) ?>",
        title:"<?= ucfirst($pathInfo['filename']) ?>",
        url: "<?= '/vvvebJs/templates/'.$pathInfo['basename'] ?>",
        assets: ['/css/fontawesome.min.css', '/css/fancybox.min.css', '/css/odometer.min.css', '/css/swiper.min.css', '/css/bootstrap.min.css', '/css/style.css', '/css/custom.css']}, 
      <?php endforeach; ?>
